...
class GroupROICrop(object):
    def __init__(self, ROI):

        # ROI:
        # INPUT  x_min, y_min, w, h;
...
class GroupRandomScaleNew(object):
    ...
    def __call__(self, img_group):
        if (len(self.interpolation) != len(img_group)):
            print("{} != {}, interpolation is  {}, image_group is {}".format(len(self.interpolation) ,len(img_group), self.interpolation, img_group))
            raise
        h, w = img_group[0][0].shape[0:2]
        scale_w, scale_h = self.size[0] * 1.0 / w, self.size[1] * 1.0 / h 
        out_images = list()
...
class GroupRandomScaleNew(object):
    ...
    def __call__(self, img_group):
        ...
        out_images = list()
        for img, interpolation in zip(img_group, self.interpolation):
            out_images.append(cv2.resize(img, self.size, interpolation=interpolation))
            if len(img.shape) > len(out_images[-1].shape):
                out_images[-1] = out_images[-1][..., np.newaxis]  # single channel image
        return out_images
